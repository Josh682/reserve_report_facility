...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Tambah Fasilitas Baru</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Lengkapi formulir berikut untuk mendaftarkan fasilitas kampus baru ke dalam sistem.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.facilities.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <select id="tipe" name="tipe" required
                            ...
                            <option value="ruang_kelas" @selected(old('tipe') === 'ruang_kelas')>Ruang Kelas</option>
                            <option value="aula" @selected(old('tipe') === 'aula')>Aula</option>
                            <option value="laboratorium" @selected(old('tipe') === 'laboratorium')>Laboratorium</option>
                            <option value="alat" @selected(old('tipe') === 'alat')>Alat</option>
                            <option value="lapangan" @selected(old('tipe') === 'lapangan')>Lapangan</option>
                        </select>
                    </div>
                    @error('tipe')
                        <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <select id="status" name="status" required
                            ...
                            <option value="aktif" @selected(old('status', 'aktif') === 'aktif')>Aktif (Dapat dipinjam)</option>
                            <option value="dalam_perbaikan" @selected(old('status') === 'dalam_perbaikan')>Dalam Perbaikan (Maintenance)</option>
                            <option value="nonaktif" @selected(old('status') === 'nonaktif')>Nonaktif (Tidak tersedia)</option>
                        </select>
                    </div>
                    @error('status')
                        <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="number" id="kapasitas" name="kapasitas" value="{{ old('kapasitas') }}" min="1"
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea id="deskripsi" name="deskripsi" rows="4"
                              ...
                              placeholder="Keterangan fasilitas, kelengkapan proyektor, AC, sound system, atau syarat pemakaian...">{{ old('deskripsi') }}</textarea>
                </div>
                @error('deskripsi')
                    <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
                @enderror
