...
<head>
    ...
    <title>@yield('title', 'Petugas Panel') — {{ config('app.name', 'Sistem Reservasi & Pelaporan Fasilitas Kampus') }}</title>
...
<body class="h-full font-sans antialiased text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-900">
    <div class="min-h-screen flex flex-col lg:flex-row">
        ...
        <div id="petugas-sidebar-backdrop" class="fixed inset-0 z-40 bg-gray-900/50 backdrop-blur-xs hidden lg:hidden" onclick="togglePetugasSidebar()"></div>

        <!-- Sidebar Navigation -->
        <aside id="petugas-sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col transition-transform duration-200 -translate-x-full lg:translate-x-0 lg:static lg:z-auto">
            <!-- Brand Header -->
            <div class="h-16 flex items-center justify-between px-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                <a href="{{ route('petugas.dashboard') }}" class="flex items-center gap-3 font-bold text-lg text-emerald-600 dark:text-emerald-400">
                    ...
                    <span class="tracking-tight text-gray-900 dark:text-white">Petugas Fasilitas</span>
                </a>
                <button type="button" class="lg:hidden text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" onclick="togglePetugasSidebar()">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
                ...
                <a href="{{ route('petugas.dashboard') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('petugas.dashboard') ? 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 font-semibold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/60' }}">
            ...
            <div class="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                <div class="flex items-center gap-3 px-2">
                    <div class="w-9 h-9 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-sm shrink-0">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0">
                <div class="flex items-center gap-3">
                    <button type="button" class="lg:hidden p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700" onclick="togglePetugasSidebar()">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                    </button>
                    <h1 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">
                        @yield('title', 'Petugas Dashboard')
                    </h1>
                </div>
